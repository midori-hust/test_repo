...
int main(){
  ...
  int enermySide, otherSide;
  int randomNum;
  int socket1 = 5, socket2 = 6;
        ...
        switch(newMesg->cmd) {
          case SIGNIN:
            ...
            if((  (strcmp(id, id1) == 0) && (strcmp(password, password1) == 0) )
                || ( (strcmp(id, id2) == 0) && (strcmp(password, password2) == 0)) ) {
            ...
            if((strcmp(id, id1) == 0) && (strcmp(password, password1) == 0)) {
              ...
              printf("%s\n", message_toString(message_construct(PICK_SIDE, DARK, "0", "0", "0")));
              if (send(fd, message_toString(message_construct(PICK_SIDE, DARK, "0", "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN) {
                printf("send() sent a different number of bytes than expected\n");
              }
              ...
              socket2 = fd;
              sprintf(temp, "%d", socket1);
              printf("%s\n", message_toString(message_construct(PICK_SIDE, LIGHT, "0", "0", "0")));
              if (send(fd, message_toString(message_construct(PICK_SIDE, LIGHT, "0", "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN) {
                printf("send() sent a different number of bytes than expected\n");
              }         
            }
            */
          ...
          break;
          case MOVE:          
            //TODO: play module here (check the socket from, and check the room which has this player to get opposite socket, then send move throught it)
          ...
          break;
          case INVITE_SEND:
          //TODO: send it to other socket
          newMesg->arg[1] = '\0';
          enermySide = atoi(newMesg->arg);
          sprintf(temp, "%d", fd);
          if (send(enermySide, message_toString( message_construct(INVITE_ACCEPT, newMesg->arg, temp, "0", "0") ), MESSAGE_MAXLEN , 0) != MESSAGE_MAXLEN ) {
            printf("send() sent a different number of bytes than expected\n");
          }
          break;
          case INVITE_ACCEPT:
          ...
          if (send(enermySide, message_toString( message_construct(PICK_SIDE, temp, "0", "0", "0") ), MESSAGE_MAXLEN , 0) != MESSAGE_MAXLEN ) {
            printf("send() sent a different number of bytes than expected\n");
          }
