					...
					case PLAYING:
						...
						do {
							...
							if (playerAnswer == RIGHT) {
								printf(
										"Good job! The player gave a right answer!\n");
								...
								if (level == MAX_LEVEL) {
									...
									send(i, &toPlayerData, sizeof toPlayerData,
											0);
									sprintf(toPlayerData.message,
											">>>Chuc mung ! Ban %s da xuat sac tra loi dung 15 cau hoi! Ban ra ve voi phan thuong %s(VND)<<<\n",
											toPlayerData.name,
