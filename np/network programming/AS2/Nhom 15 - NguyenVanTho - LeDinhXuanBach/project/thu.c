...
int checkConnectionLost_connectList()
{
  int i=0,k=0;
  for(i=0;i<indexMax;i++)
    {
  ...
  return k;//number of connections lost
}
int checkConnectionLost()
{
  int i=0,k=0;
  for(i=0;i<indexMax;i++)
    {
...
char *argv[];
{
  ...
  //gdk_threads_enter ();
	...
	int k=0;
	printf("------------------Part 1--------------------\n");
	pthread_create (&yes_tid, NULL,lostThread,NULL);
	while(i<total)
	  {
	    ...
	    if(strcmp(listQA[cqa+1].p,"P2_")==0&&k==0)
	      {
		printf("------------------Part 2--------------------\n");
		k=1;
	      }
