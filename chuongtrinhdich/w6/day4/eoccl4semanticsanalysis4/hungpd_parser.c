...
void compileForSt(void) {
  ...
  Type *temp;//Cho bien chay
  Type *temp1;//Bieu thuc thu 1
  Type *temp2;//Bieu thuc thu 2
  eat(KW_FOR);
  //eat(TK_IDENT);
  // check if the identifier is a variable
  //checkDeclaredVariable(currentToken->string); --> Khong can vi trong compileLvalue da check
  temp=compileLValue();
  eat(SB_ASSIGN);
  temp1=compileExpression();
  checkTypeEquality(temp,temp1);
  eat(KW_TO);
  temp2=compileExpression();
  checkTypeEquality(temp,temp2);
  eat(KW_DO);
  compileStatement();
}
