...
void eat(TokenType tokenType) {
		if (lookAhead->tokenType == tokenType) {
				//    printToken(lookAhead); // for debugging
				scan();
...
Type* compileLValue(void) {
		...
		case OBJ_PARAMETER:
				...
				if(var->paramAttrs->kind == PARAM_VALUE) {
						genParameterAddress(var); // Neu la tham tri >> lay gia tri dia chi cua cai ban sao va thay doi gia tri cua ban sao thong qua dia chi do
				}
				else genParameterValue(var); // Neu la tham bien >> lay gia tri tu dia chi kia va thay doi tet ga thong qua dia chi kia
...
void compileCallSt(void) {
		...
		if (isPredefinedProcedure(proc)) {
				...
				genINT(4); // De bo qua 4 gia tri RV, DL, RA, SL
				compileArguments(proc->procAttrs->paramList);
...
void compileIfSt(void) {
		Instruction* fjInstruction;  // False Jump
		Instruction* jInstruction;	 // Jump
		...
		case TK_IDENT:
				...
				case OBJ_PARAMETER:
						...
						if (obj->paramAttrs->kind == PARAM_REFERENCE)
								genLI(); //Vi la tham bien >> load indirect (genius)
...
Type* compileIndexes(Type* arrayType) {
		...
		while (lookAhead->tokenType == SB_LSEL) {
				...
				while(element->typeClass == TP_ARRAY){
						addressElement *= element->arraySize;
						element = element->elementType;
				}
				genLC(addressElement);
				genML();
				genAD();
