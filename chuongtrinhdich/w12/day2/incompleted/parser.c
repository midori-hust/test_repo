...
void eat(TokenType tokenType) {
  if (lookAhead->tokenType == tokenType) {
    //    printToken(lookAhead); // No need to do this thing
    scan();
...
Type* compileLValue(void) {
  ...
  case OBJ_VARIABLE:
    ...
    genVariableAddress(var); // generate the LA instruction >> the virtual stack uses to do its job. It must be LA because its value is changed >> pass by reference
    ...
    break;
  case OBJ_PARAMETER:
    // TEMPORARY: halt the program (pause, okay, finally I got it!)
    genHL();
  ...
  } // Left Value is divided into 3 kinds of object: variable, parameter and function (just in its own scope)
...
void compileIfSt(void) {
  ...
  compileCondition();
  eat(KW_THEN);
  fjInstruction = genFJ(DC_VALUE); // #define DC_VALUE 0 (instruction.h), in this case, jump to label 0 (false jump)
...
void compileForSt(void) {
  ...
  Instruction * fjInstruction;  // False Jump Instruction
  Type* varType;
  Type *type;
