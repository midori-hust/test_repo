...
String* cloneArray(String *src, int amount) {
	...
	if((result = mallocStringStruct(amount)) == NULL) {
		return NULL;
	}

	for(i = 0; i < amount; i++) {
		strcpy(result[i].content, src[i].content);
	}
...
int solve(FILE *f_domain, FILE *f_user, FILE *fout1, FILE *fout2) {
	String *emailList, *result, *cloneList;
	int rows;
	clock_t begin, end;
	...
	cloneList = cloneArray(emailList, MAXSIZE);
	begin = clock();
	sort(emailList, 0, MAXSIZE - 1);
	end = clock();
	printf("3 way sort: %lf second\n", (double) (end - begin) / CLOCKS_PER_SEC);
	
	begin = clock();
	qsort(cloneList, MAXSIZE, sizeof(String), myCompare);
	end = clock();
	printf("2 way sort: %lf second\n", (double) (end - begin) / CLOCKS_PER_SEC);
