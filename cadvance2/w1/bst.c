...
void InsertNode(char *key, Node** Root, int point) {
  if(*Root == NULL) {
    ...
    printf("The new node at address: ");
    *Root = (Node*)malloc(sizeof(Node));
    ...
    if(strcmp(key, (*Root)->name) < 0)
      InsertNode(key, &(*Root)->left, point);
    else if(strcmp(key, (*Root)->name) > 0)
      InsertNode(key, &(*Root)->right, point);
    else {
      (*Root)->point += point;
    }
}
  ...
  if(*Root != NULL) {
    if(strcmp(key, (*Root)->name) <0) {
      ...
      *Root = (*Root)->left;
      ...
      strcpy((*Root)->name, DeleteMin(&(*Root)->right));
    }
  }
}
